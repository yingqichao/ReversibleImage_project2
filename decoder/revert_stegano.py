...
class RevertStegano(nn.Module):
    def __init__(self,config=GlobalConfig()):
        ...
        self.downsample_3 = nn.Sequential(
            # SingleConv(128, out_channels=64, kernel_size=1, stride=1, dilation=1, padding=0),
    ...
    def forward(self, p, crop_mask):
        ...
        down4_cat = torch.cat((down8, down7, down6, down5, down4), 1)
        up4 = self.upsample4(down4_cat)
        up4_cat = torch.cat((down8, down7, down6, down5, down4, up4), 1)
        up3 = self.upsample3(up4_cat)
        up3_cat = torch.cat((down8, down7, down6, down5, down4, up4, up3), 1)
        up2 = self.upsample2(up3_cat)
        up2_cat = torch.cat((down8, down7, down6, down5, down4, up4, up3, up2), 1)
